...
export class CdkAppsyncEventbridgeSnsStepfunctionsBackendStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const http_datasource = api.addHttpDataSource('bookmarkapp-ds',
    ...
    //mutations
    const mutations = ["addBookmark", "delBookmark"];
    mutations.forEach((thatMutation) => {
      let details = `\\\"id\\\": \\\"$ctx.args.id\\\"`;

      if(thatMutation === "addBookmark") {
        details = `\\\"id\\\": \\\"$ctx.args.id\\\", \\\"title\\\":\\\"$ctx.args.title\\\", \\\"url\\\":\\\"$ctx.args.url\\\", \\\"description\\\":\\\"$ctx.args.description\\\"`;
      }
    ...
    const dynamodbLambda = new lambda.Function(this, 'bookmarkapp-dynamodbLambda', {
...
//**************************DYNAMODB**************************/
    //creating table
    const myTable = new ddb.Table(this, 'bookmarkapp-table', {
...
//**************************SNS**************************/ 
    ...
    const snsTopic = new sns.Topic(this, 'bookmarkapp-snsTopic');

    const phoneNumber = "+923353089102";

    //following command subscribes our email & phone to the SNS topic ( whenever a topics comes then goes to phone )
...
//*********************ROLES SNSLAMBDA**********************/
    ...
    //creating lambdafunctions
    const snsLambda = new lambda.Function(this, 'bookmarkapp-snsLambda', {
      runtime: lambda.Runtime.NODEJS_10_X,
      code: lambda.Code.fromAsset('lambda'),
      handler: 'sns.handler',
...
//*********************STEPFUNCTIONS**********************/
    ...
    const stepFnStateMachine = new stepFunctions.StateMachine(this, 'bookmarkapp-stateMachine', {
      definition: chain
    });
//*********************STEPFUNCTIONS**********************/
    ...
    const rule = new events.Rule(this, 'appsyncEventbridgeRule', {
      ruleName: 'bookmarkapp-appsyncEventbridgeRule',
      description: 'created for appSyncEventbridge',
      eventPattern: {
        source: ["bookmarkapp-events"],
        //every event that has source = "bookmarkapp-events" will be sent to our lambda
      },
